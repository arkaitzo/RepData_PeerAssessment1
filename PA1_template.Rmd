...
```
- The mean number of steps taken per day is **`r format(steps.mean, scientific = FALSE)`** and the median is **`r steps.median`**.
...
```
- **`r max.interval`** is the 5-minute interval that contains the maximum number of steps.
...
```
- All the **`r total.na`** missing values in the dataset belong to the attribute `steps`.
...
```
- The mean number of steps taken per day is **`r format(steps.mean.v2, scientific = FALSE)`** and the median is **`r format(steps.median.v2, scientific = FALSE)`**.
...
- There are no empty dates now in the histogram and, in addition, the total number of steps changed from **`r sum(temp.v1$steps)`** to **`r format(sum(temp.v2$steps), scientific = FALSE)`**.
